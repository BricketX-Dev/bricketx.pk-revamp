...
export default function Departments() {
  ...
  useGSAP(() => {
    // 1. Setup mouse parallax bindings (Safe globally, driven by mouseMove)
    ...
    // 3. MatchMedia for Responsive GSAP Stacking (Desktop only)
  ...
  const handleMobileScroll = (e: UIEvent<HTMLDivElement>) => {
    ...
    const activeIndex = Math.min(
      ...
      Math.max(1, Math.round(progress * (departmentData.length - 1)) + 1)
    );
  ...
  return (
    ...
    >
      ...
      >
        {/* Header Strip */}
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 shrink-0 mb-8 md:mb-4">
        ...
        <div 
          ...
          className="flex md:block w-[calc(100%+3rem)] -ml-6 px-[10vw] md:w-full md:ml-0 md:px-0 overflow-x-auto md:overflow-visible snap-x snap-mandatory md:snap-none gap-4 md:gap-0 pb-6 md:pb-0 relative md:max-w-5xl h-[520px] md:h-[510px] my-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]"
        >
          {departmentData.map((dept, idx) => {
            ...
            return (
              <div
                key={dept.num}
                className="dept-card-wrapper group relative md:absolute shrink-0 w-[80vw] md:w-full h-full md:inset-0 snap-center rounded-3xl border border-[#C39967]/30 hover:border-[#C39967]/70 bg-white/85 backdrop-blur-2xl shadow-[0_20px_60px_-15px_rgba(195,153,103,0.25)] hover:shadow-[0_30px_80px_-15px_rgba(195,153,103,0.45)] transition-[box-shadow,border-color] duration-500 ease-out overflow-hidden flex flex-col justify-between p-7 md:p-12 will-change-transform"
                style={{ zIndex: idx + 10 }}
              >
                ...
                <div className="relative z-10 flex items-center justify-between border-b border-[#C39967]/20 group-hover:border-[#C39967]/40 transition-colors duration-500 pb-5">
                  <div className="flex items-center gap-2 md:gap-3">
                    <span className="font-mono text-[10px] md:text-xs font-bold text-[#C39967] tracking-wider uppercase">
                      {dept.tag}
                    </span>
                  ...
                  <div className="inline-flex items-center gap-1.5 md:gap-2 px-2 md:px-3 py-1 rounded-full bg-white border border-[#C39967]/30 text-[10px] md:text-xs font-mono font-medium text-[#18181B] shadow-sm whitespace-nowrap">
                    <span className="w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-[#C39967] animate-pulse" />
                    {dept.badge}
                  </div>
                </div>
                ...
                <div className="relative z-10 my-auto py-2">
                  <div className="flex items-center gap-4 mb-4">
                    <div className="w-12 h-12 md:w-14 md:h-14 rounded-2xl bg-[#FAF5EE] group-hover:bg-[#C39967] border border-[#C39967]/40 group-hover:border-[#C39967] shadow-sm group-hover:shadow-[0_10px_25px_-5px_rgba(195,153,103,0.6)] flex items-center justify-center text-[#C39967] group-hover:text-white group-hover:-translate-y-1.5 transition-all duration-500 ease-out shrink-0">
                      <Icon size={24} className="group-hover:scale-110 transition-transform duration-500 ease-out" />
                    </div>
                    <div>
                      <span className="text-[10px] md:text-xs font-mono text-[#5E646D] group-hover:text-[#C39967] transition-colors duration-300 uppercase tracking-wider block">
                        Engine Division {dept.num}
                      </span>
                      <h3 className="text-2xl md:text-5xl font-black tracking-tight text-[#18181B]">
                        {dept.name}
                      </h3>
                    </div>
                  </div>

                  <p className="text-[#5E646D] text-sm md:text-lg max-w-2xl leading-relaxed font-normal">
                    {dept.description}
                  </p>
                </div>
                ...
                <div className="relative z-10 pt-5 border-t border-[#C39967]/20 group-hover:border-[#C39967]/40 transition-colors duration-500 flex flex-col md:flex-row md:items-center justify-between gap-5">
                  <div className="flex flex-wrap gap-2">
                    {dept.capabilities.map((cap, cIdx) => (
                      <span 
                        key={cIdx} 
                        className="inline-flex items-center gap-1.5 px-2.5 py-1 md:px-3 md:py-1.5 rounded-lg bg-white/70 hover:bg-white backdrop-blur-sm border border-[#C39967]/20 hover:border-[#C39967]/60 hover:shadow-[0_4px_12px_-4px_rgba(195,153,103,0.4)] hover:-translate-y-0.5 transition-all duration-300 cursor-default text-[10px] md:text-xs font-semibold text-[#18181B]"
                      >
                        <CheckCircle2 size={12} className="text-[#C39967]" />
                        {cap}
                      </span>
                    ))}
                  </div>

                  <div className="flex md:items-center gap-4 md:gap-6 font-mono text-xs text-[#5E646D] shrink-0 justify-between md:justify-end w-full md:w-auto">
                    {dept.stats.map((stat, sIdx) => (
                      <div key={sIdx} className="text-left md:text-right">
                        <span className="block text-[9px] md:text-[10px] uppercase text-[#C39967] tracking-wider">{stat.label}</span>
                        <span className="font-bold text-[#18181B] text-xs md:text-sm group-hover:text-[#C39967] transition-colors duration-300">{stat.val}</span>
                      </div>
                    ))}
                  </div>
                </div>
